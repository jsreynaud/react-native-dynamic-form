...
export default class TagsInput extends React.Component {
  ...
  handleDelete = index => {
    ...
    this.setState({customHeight: this.state.customHeight - 10})
  };
  ...
  handleAddition = contact => {
    ...
    this.setState({ tagsSelected: this.state.tagsSelected.concat([contact]) }, () => {
      ...
      console.log("ADD", this.state.tagsSelected.length);
      const { onSelect } = this.props;
      onSelect(this.state.tagsSelected);
    });
    this.setState({textType:""});
    this.setState({customHeight: this.state.customHeight + 20});
    this.setState({isClicked: false});
  };

  onChangeText = text => {
      const { onFilterFunc } = this.props;
      onFilterFunc(text);
  ...
  render() {
    ...
    return (
      <View style={[styles.container,{opacity: tagDisable == 'none' ? 0.7 : 1, height: Platform.OS === "android" ? this.state.isClicked ? 400 + this.state.customHeight : this.state.customHeight :150}]}>
       <LabelError
          label={label}
          error={error}
        />
        <View pointerEvents={tagDisable} style={styles.autocompleteContainer}>
          <AutoTags
            ...
            // onChangeText={this.props.onFilterFunc}
